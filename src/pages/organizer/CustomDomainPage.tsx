...
export default function CustomDomainPage() {
  ...
  const selectedSlug = tournaments.find((t) => t.id === selectedId)?.slug ?? "";
  ...
  const load = async () => {
    ...
    setIsLoading(true);
    const { data } = await supabase.from("custom_domains").select("*").eq("tournament_id", selectedId).maybeSingle();
    setDomain(data as CustomDomain | null);
    setIsLoading(false);
  };
      ...
      {isLoading ? (
        ...
        <>
          <section className="rounded-card border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
            <h2 className="font-heading text-sm font-semibold text-[var(--color-heading)]">Free public website</h2>
            <p className="mt-1 text-xs text-[var(--color-muted)]">
              Included free with your plan — no domain purchase needed.
            </p>
            <p className="mt-2 flex items-center gap-2 text-sm text-[var(--color-text)]">
              {selectedSlug ? getTournamentUrl(selectedSlug) : "Not set"}
              {selectedSlug && (
                <button onClick={() => navigator.clipboard.writeText(getTournamentUrl(selectedSlug))} aria-label="Copy link" className="text-[var(--color-muted)] hover:text-[var(--color-primary)]">
                  <Copy size={14} />
                </button>
              )}
            </p>
          ...
          <section className="space-y-4 rounded-card border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
            <div>
              <h2 className="font-heading text-sm font-semibold text-[var(--color-heading)]">Custom domain (optional)</h2>
              <p className="mt-1 text-xs text-[var(--color-muted)]">
                Bring your own domain to replace the free public website URL above. This requires a domain you've
            ...
            {domain ? (
              <div className="space-y-4">
                ...
                <div className="rounded-lg bg-[var(--color-surface-secondary)] p-4 text-xs">
                  ...
                  <p className="text-[var(--color-muted)]">Add a CNAME record pointing your domain to <code>cname.vercel-dns.com</code> (see your Vercel project's Domains settings for the exact target), then add this TXT record to verify ownership:</p>
